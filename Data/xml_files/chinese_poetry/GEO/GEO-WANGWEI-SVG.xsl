...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template match="place" mode="css">
        ...
        #<xsl:value-of select="@name"/>-d{
        display: <xsl:if test="contains(//group/@ht, @name)">block</xsl:if><xsl:if test="contains(//group/@rp, @name)">block</xsl:if><xsl:if test="contains(//group/@lp, @name)">block</xsl:if>}
    </xsl:template>
    <xsl:template match="group/place">
        ...
        <text x="{$xpos}" y="20" text-anchor="start" font-size="8pt"><xsl:value-of select="@name"/></text>
