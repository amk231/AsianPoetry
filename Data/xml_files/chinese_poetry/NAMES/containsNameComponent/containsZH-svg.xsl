    ...
    <xsl:template match="/">
        <svg>
            <g transform="translate(200,450) scale(1.5)">
                ...
                <xsl:variable name="average" select="//types[@eng='ZH']/type[@type='total-total'] * 100"/>
                
                
                
                <xsl:for-each select="//types[@eng='ZH']/type[not(@type='total-total')]">
                    <xsl:variable name="xpos" select="55 * (position() - 1) + 30"/>
                    <rect x="{$xpos}" y="-{. * 100 }" height="{. * 100}" width="25">
                    ...
                    <text x="{$xpos}" y="-{(. * 100) + 10}"><xsl:value-of select="format-number((. * 100),'#.#')"/><xsl:if test=". = 0">0</xsl:if>%</text>
                ...
                <text x="600" y="20" text-anchor="end">Zhi</text>
